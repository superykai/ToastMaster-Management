...
export class AssignmentHistoryComponent implements OnInit, OnDestroy {
            ...
            if (this.numOfHistoriesNeed > 0) {
              for (let i = totalHistories - 1; i >= totalHistories - this.numOfHistoriesNeed; i--) {
                ...
                for (const [key, val] of Object.entries(this.listOfHistories[i] as Array<ClubMeetingModel>)) {
                  ...
                  //   delete val.key;
                  if (key !== 'key'){
                    const result = {key: this.listOfHistories[i].key + '/' + key, date: val.date,
                      roleId: val.roleId, roleName: '', assignedTo: val.assignedTo,
                      email: '', memberName: '', memo: '', sortIndex: 0, timeLimit: '' };
                    history.push(result);
                  }
                  if (val.date) {
                    this.historyDates.push(val.date);
                  } // this is for when add new history table, need to avoid choose these existing dates
                }
                // history.pop();
                this.histories.push(history);
                if (this.histories[0][0]) {
                  this.histories.sort((a, b) => new Date(b[0].date).valueOf() - new Date(a[0].date).valueOf());
                } // latest appear on top
              }
  ...
  refreshUIDataSource(historyData){
    ...
    historyData.forEach(history => {
      const oneSource = new MatTableDataSource(history);
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result.event === 'Delete'){
        this.histories.forEach(hr => {
          const item = hr.find(h => h.date === result.data.date && h.assignedTo === result.data.assignedTo
            && h.roleId === result.data.roleId);
      ...
      if (result.event === 'Add'){
        ...
        if (result.from === 'from-new-history'){
          ...
          const newHistory = this.db.database.ref('assignmentHistory').push(); // this wont call to server
          const newRecord = new Array({key: newHistory.key, assignedTo: 0, date: Util.GetFormattedDate(result.data.date),
            roleId: 0, roleName: '', email: '', memberName: '', memo: '', sortIndex: 0, timeLimit: ''});

          newHistory.set(newRecord);
